  ...
  return (dispatch, getState: () => IReduxState) => {
    ...
    if (previousCityCoordinates) {
      DirectionsService.get([previousCityCoordinates, city.getCoordinatesArray()], 'driving').then((response: any) => {
        ...
        dispatch(citiesActions.updateOne({
          cityId: city.id, update: {
...
const rescheduleCity = (cityId, startDate, endDate) => {
  return (dispatch, getState: () => IReduxState) => {
    ...
    savedCitiesAfter.forEach((city, i) => {
      ...
      if (shouldUpdate) {
        dispatch(updateDirections(city));
      }
    })
  }
};
