...
export class CityService {
  public static byCoordinates(longitude, latitude): Promise<City> {
    return new Promise((resolve, reject) => {
      axios.get(`https://api.mapbox.com/geocoding/v5/mapbox.places/${longitude},${latitude}.json?types=place&access_token=${process.env.BONTRIP_MAP_KEY}`)
  ...
  public static byName(searchText) {
    return new Promise((resolve, reject) => {
      ...
      if (searchText) {
        axios.get(`https://api.mapbox.com/geocoding/v5/mapbox.places/${searchText}.json?types=place&access_token=${process.env.BONTRIP_MAP_KEY}`)
            .then(function (response) {
